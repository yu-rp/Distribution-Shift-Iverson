...
def main():
    ...
    assert os.path.isfile(opt.init_img) # 需要是一张图
    ...
    t_enc = int(opt.strength * opt.ddim_steps) # 本质上也是控制 t 这很好
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                tic = time.time()
                all_samples = list()
                for n in trange(opt.n_iter, desc="Sampling"): # 采样多少次
                    for prompts in tqdm(data, desc="data"):
                        uc = None
                        if opt.scale != 1.0: # 我们如果保证 prompt 是空的 那么其实 这个是1 就行
