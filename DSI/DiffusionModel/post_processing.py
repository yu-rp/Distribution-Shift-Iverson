...
def pp_convexcombination(*args, **kwargs):
    ...
    sample:     the sample at time index i 
    origin:     the image with noise
    alpha:      convex comb coefficient, before the sample image
    """
...
def pp_hardness(*args, **kwargs):
    ...
    return alpha * sample + (1 - alpha) * referece 
...
def pp_frequencydomaincombination(*args, **kwargs):
    ...
    sample:     the sample at time index i 
    origin:     the image with noise
    range:      the range preserved in the sample, this is the only part generated by diffusion model
    ...
    up = int(size[0] * range[1] /2), int(size[1] * range[1]/2)
    down = int(size[0] * range[0] /2),  int(size[1] * range[0]/2)

    mid = int(size[0] / 2),int(size[1] / 2)

    output_freq = referece_freq.copy()
    output_freq[:,:,mid[0] - up[0]:mid[0] + up[0],mid[1] - up[1]:mid[1] + up[1]] = sample_freq[:,:,mid[0] - up[0]:mid[0] + up[0],mid[1] - up[1]:mid[1] + up[1]].copy()
    output_freq[:,:,mid[0] - down[0]:mid[0] + down[0],mid[1] - down[1]:mid[1] + down[1]] = referece_freq[:,:,mid[0] - down[0]:mid[0] + down[0],mid[1] - down[1]:mid[1] + down[1]].copy()
...
def pp_lowpass(*args, **kwargs):
    ...
    return (reference_u + sample_highfreq) 
